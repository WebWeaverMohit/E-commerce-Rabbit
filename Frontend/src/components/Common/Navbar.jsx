    ...
    <>
      <div className='container mx-auto flex items-center justify-between py-4 px-6'>
        ...
        <div className='flex items-center space-x-4'>
          <Link className='block bg-black px-2 py-1 rounded text-sm text-white' to={'/admin'}>Admin</Link>
          <Link to="/Profile" className='hover:text-black'>
            <HiOutlineUser className='h-6 w-6 text-gray-700' />
          </Link>
          <button onClick={toggleCartDrawer} className='relative hover:text-black'>
            <HiOutlineShoppingBag className='h-6 w-6 text-gray-700' />
            <span className='absolute bg-[#ea2e0e] text-white text-xs rounded-full px-2 -top-1 py-0.5'>4</span>
          </button>
          <div className='overflow-hidden'></div>
          <SearchBar />
          <button onClick={toggleNavDrawer}>
            <HiBars3BottomRight className='md:hidden h-6 w-6 text-gray-700' />
          </button>
        </div>
      </div>
      ...
      <div className={`fixed top-0 left-0 w-3/4 md:w-1/3 border-r border-gray-500 h-full bg-white shadow-lg transform transition-transform duration-300 z-50 ${navDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}>
